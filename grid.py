...
def move_agente(num_linha, linha, agente):
  for i in range(agente.position[1], len(linha)):
    ...
    if i + 1 < len(linha) and linha[i + 1] in [0, 'G']:
      agente.move_right()
      pygame.draw.rect(screen, color, pygame.Rect(agente.position[1] * 100, agente.position[0] * 100, 100, 100))
...
if __name__ == "__main__":
    ...
    while not done:
        ...
        pygame.draw.rect(screen, color, pygame.Rect(agente.position[0], agente.position[1], 100, 100))
